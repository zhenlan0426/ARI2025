...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor, interpolate_pos_encoding=False) -> torch.Tensor:
        def get_position_ids(h, w):
            """
            Generate position_ids for a custom image size h x w to use with pretrained position embeddings.
            Maps custom patch indices to the top-left portion of the original grid in row-major order.
            ...
            Returns:
                torch.Tensor: Position IDs of shape (1, nh * nw) for use in position_embedding.
            """
            patch_size = 14
            grid_size = 64  # e.g., 64
            nh = h // patch_size
            nw = w // patch_size
            # Generate position_ids in row-major order: [0, 1, 2, ..., 64, 65, 66, ...]
            position_ids = torch.tensor([i * grid_size + j for i in range(nh) for j in range(nw)], dtype=torch.long, device='cuda')
            return position_ids.unsqueeze(0)  # Shape: (1, nh * nw)
        
        _, _, height, width = pixel_values.shape
        target_dtype = self.patch_embedding.weight.dtype
        patch_embeds = self.patch_embedding(pixel_values.to(dtype=target_dtype))  # shape = [b, c, h, w]
        embeddings = patch_embeds.flatten(2).transpose(1, 2) # [b, h * w, c]
